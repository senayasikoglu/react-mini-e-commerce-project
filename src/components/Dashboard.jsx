...
function Dashboard() {
    const fragances = products.filter(product => product.category === "fragrances" || product.category === "skincare");
                ...
                {productsToDisplay.map((perfumeDetails) => {
                    return (
                        <div key={perfumeDetails.id} className="perfume-item">
                            ...
                            <img className="item-img" alt="perfume image" src={perfumeDetails.images[0]} />
                            <p>${perfumeDetails.price}</p>

                            <button className="delete-btn" onClick={() => { deleteProduct(perfumeDetails.id) }}>Delete</button>
                        </div>
                    );
                })}
            </div>
